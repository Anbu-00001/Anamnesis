//! The fictional demo ledger.
//!
//! A visitor decides in about two minutes, and a brand-new ledger has nothing to
//! show — so `ana demo` builds this one in a temp directory and reports on it,
//! never touching the user's own file.
//!
//! The data encodes a very human pattern — competent discrimination wrapped in
//! systematic overconfidence — so the mirror has something honest to reflect
//! back. None of these are real predictions; they are a fictional forecaster's
//! year, chosen to make the report legible.
